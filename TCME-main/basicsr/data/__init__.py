...
dataset_opt (dict):数据集选项。它包含以下键:
阶段(str): 'train'或'val'。
num_worker_per_gpu (int):每个GPU的worker数。
batch_size_per_gpu (int):每个GPU的训练批大小。
num_gpu (int): gpu数量。仅在列车阶段使用。
默认值:1。
dist (bool):是否进行分布式训练。只在火车上使用
阶段。默认值:False。
sampler (torch.utils.data.sampler):数据采样器。默认值:没有。
    """
