...
class BaseModel():
    ...
    def validation(self, dataloader, current_iter, tb_logger, save_img=False, rgb2bgr=True, use_image=True):
        ...
        Args:
            ...
            参数:
                dataloader (torch.utils.data.DataLoader):验证数据加载程序。
                current_iter (int):当前迭代。
                tb_logger (tensorboard logger): tensorboard logger。
                save_img (bool):是否保存图片。默认值:False。
                rgb2bgr (bool):是否使用rgb2bgr保存图像。默认值:真正的
                use_image (bool):是否使用保存的图像来计算指标(PSNR, SSIM)，如果不是，则直接使用网络输出的数据。默认值:真正的
                ”“”
